...
const writeExamplePage = async (
  ...
  const page =
    ...
    insertTemplate(
      tsxTemplate,
      {
        id,
        title,
        parentUrl: parentUrl.replace(/__examples/g, ''), // markdown with "__examples" suffix doesn't have a regular parent page
      },
      scriptImports.join('')
    ).replace(
      pageContentComponentSignature,
  ...
  const html = insertTemplate(htmlTemplate, {
    id,
    title,
    parentUrl: parentUrl.replace(/__examples/g, ''), // pages that end in __examples link to a different page so remove the examples title,
    scriptUrl,
    serverExampleHTML,
    linkBackPageName,
  })
      ...
      case 'JSXElement': {
        ...
        if (node.openingElement.attributes) {
          newHTML += node.openingElement.attributes
            .map((attr, index, attrs) => {
              let attributeName =
                attr.name.name.type === 'JSXIdentifier'
                  ? attr.name.name.name
                  : attr.name.name;

              // the parser has lots of bugs. It splits attributes if they're namespaced :(
              // many hacks to put attributes back together again.
              if (
                elementName.toLowerCase() === 'use' &&
                attributeName === 'href'
              ) {
              ...
              if (
                // bugs in parser... it splits xmlns:xlink into two attributes
                attrs[index + 1] &&
                attrs[index + 1].name.name.type === 'JSXIdentifier'
              ) {
                attrString += ':';
                return attrString;
              }
