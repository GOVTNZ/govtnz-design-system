...
const normalize_selector_1 = __importDefault(require("normalize-selector"));
const lodash_1 = require("lodash");
const progress_1 = __importDefault(require("progress"));
const ds_common_1 = require("@govtnz/ds-common");
const normalize_1 = require("./normalize");
const utils_1 = require("./utils");
const svg_to_dataurl_1 = __importDefault(require("svg-to-dataurl"));
const ONE_REM_IN_PIXELS = 16; // in our DS
exports.normalizeReleaseVersionGovUk = async (upstreamReleaseVersion) => {
    let components = [];
    ...
    for (let i = 0; i < upstreamReleaseVersion.components.length; i++) {
        const component = upstreamReleaseVersion.components[i];
        const metaTemplateDefs = await exports.govukToMetaTemplateInput(component.id, exports.removeGovUkIdentifiers(component.html), css, component.version);
        metaTemplateDefs.forEach(({ id, html, css, message }) => {
            if (id !== normalize_1.toId(id)) {
                // id should be normalized by now...
                throw new Error(`${__filename}: Bad id generated of ${id} (vs ${normalize_1.toId(id)}). Current message was: ${message}`);
            }
        ...
        bar.tick();
        utils_1.gc(); // JSDOM can use a lot of memory. GC time after we've used it.
        await ds_common_1.sleep(1); // give GC a bit of time
...
exports.removeGovUkIdentifiers = (data) => {
    ...
    thingsThatShouldNeverAppear.forEach(thingThatShouldNeverAppear => {
        ...
        if (found) {
            ...
            throw Error(`Removal of "${thingThatShouldNeverAppear}" failed because it matched "${found}" from "${nonUk.substring(index - 20, index + 20)}". Was: ${data.replace(/[\n\r]/g, ' ')}`);
        }
    });
    return nonUk;
};
exports.govukToMetaTemplateInput = async (oldTemplateId, oldHTML, css, semVer) => {
    let html = oldHTML;
    let message = '';
    let id = oldTemplateId;
    const additionalPrefixesToBypassNamespacing = [];
    let cssNamespace;
    ...
    return additionalTemplates.map(template => {
        if (template.id.match(/H[1-6]/)) {
            ...
            return {
                ...template,
                id: normalize_1.toId(template.id),
                css,
                message
            };
        }
        return exports.normalizeGovUkTemplate({
            ...template,
            css,
            cssNamespace,
            message,
...
exports.updateCSS = (css, rules) => {
    const newRules = rules.map(rule => {
        const newRule = [
            rule[0],
            utils_1.splitSelectors(rule[1]).map(selectorPart => normalize_selector_1.default(selectorPart)),
            rule[2] && utils_1.cssPropertiesToObject(rule[2]),
            rule[3]
        ];
        return newRule;
    });
    const matchedRules = newRules.map(newRule => newRule[1].map(s => ({
        replace: newRule[2] &&
    ...
    const newCSS = normalize_1.setCSSValues(css, ({ atRule, selector, name, value }) => {
        let govUkMediaCategory = atRuleToGovMediaCategory(atRule);
        for (let i = 0; i < newRules.length; i++) {
            ...
            for (let x = 0; x < rule[1].length; x++) {
                ...
                const normalizedSelector = normalize_selector_1.default(selector);
                if (ruleSelector === normalizedSelector) {
                    const replaceValue = rule[2] && rule[2][name];
    ...
    matchedRules.forEach((objs, i) => {
        objs.forEach((obj, x) => {
            const unusedReplaceStyles = obj.replace &&
    ...
    if (unmatchedRules.length > 0) {
        ...
        throw Error(`${__filename} given CSS rules to match that didn't match anything. Unmatched CSS rules were ${JSON.stringify(unmatchedRules, null, 0)}`);
    }
    return newCSS;
};
const atRuleToGovMediaCategory = (atRule) => {
    switch (atRule.trim() // strangely .trim() is required because atRule can be "@font-face " with a trailing space!
    ) {
        case '@media print':
            return 'print';
...
const govUKToGovtNZCSS = async (oldCSS) => {
    let css = exports.removeGovUkIdentifiers(oldCSS);
    ...
    // smallest one...
    const headingS = await normalize_1.filterCSSByClassName('g-heading-s', css);
    ...
    css = exports.updateCSS(css, [
        ...
        [
            'all',
            '.g-select',
            'border: 1px solid #000000; height: auto;',
            ` -moz-appearance: none;
        ...
        background-image: url('${svg_to_dataurl_1.default(`<?xml version="1.0" encoding="utf-8"?>
...
const govUkClassNameToGenericClassName = (namespace, className, originalIds, fullMatch) => {
    let g;
...
exports.normalizeGovUkTemplate = ({ id, html, css, cssNamespace, message, additionalPrefixesToBypassNamespacing }) => {
    // FIXME: Replace with a real HTML/CSS parser-based replacement
    const normalizedId = normalize_1.toId(id);
    const newCssNamespace = cssNamespace || lodash_1.camelCase(normalizedId);
    ...
    const namespacedHTML = html.replace(/g-([A-Za-z0-9-_]+)/g, (all, p1) => {
        ...
        const { newMessage, replacement } = govUkClassNameToGenericClassName(newCssNamespace, p1, [id, newCssNamespace, ...additionalPrefixesToBypassNamespacing], all);
        if (newMessage) {
            message += newMessage;
        }
        return replacement;
...
//# sourceMappingURL=normalize-govuk.js.map