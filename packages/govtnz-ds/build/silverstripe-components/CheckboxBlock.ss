<div class="g-checkboxes__item<% if $errorId %>  g-checkboxBlock-form-group--error<% end_if %>"> 
  ...
  <label class="g-checkboxBlock-label g-checkboxes__label"<% if $id %> for="{$id}"<% end_if %>> {$label}
</label> 
    ...
    <div class="g-checkboxBlock-error-message"<% if $errorId %> id="{$errorId}"<% end_if %>> 
