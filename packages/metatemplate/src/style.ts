...
export const replaceCSSVariables = (
  ...
  const cssProperty = (
    ...
    if (cssVariables) {
      cssVariables.forEach((cssVariable: CSSVariablePattern) => {
        ...
        //   }
        //   v += `${propertyName} ${important}; var(--${cssVariable.id}, ${cssVariable.defaultValue}) ${important}`;
        // } else
  ...
  const renderCSS = (node) => {
    ...
    switch (node.type) {
      ...
      default: {
        throw Error(
          `MetaTemplate: Unrecognised CSS node type "${node.type}". Please contribute your CSS as a test case to the project.`
        );
      }
    }
    return response;
