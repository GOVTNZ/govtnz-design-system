...
*/
async function getCSSRules(children, options, matchedCSS) {
...
async function getCSSRulesByElement(el, options, matched) {
  ...
  for (let i in sheets) {
    ...
    if (sheetIsAllowed(sheet, options)) {
      const matchedCSSRule = await _filterCSSRulesByElement(el, sheet.rules || sheet.cssRules, options, matched[i] || {});
  ...
  for (let i in rules) {
    ...
    if (rule.selectorText) {
      ...
      if (ruleIsAllowed(sanitisedSelector, options)) {
        ...
        await Promise.all(selectors.map(async selector => {
          ...
          try {
            ...
            normalizedSelector = selector.replace(/\\:/g, unique).trim(); // START OF NORMALIZATION
            ...
            normalizedSelector = normalizedSelector.replace(/^.*[\s]/, "") // regex 'greedy' selector anchored to string start, searching for whitespace to convert (eg) ".a + .b + .c" to ".c". Delete any conditions on hierarchical (adjacent sibling / descendent etc.) selectors because they could match and that's close enough to warrant including it.
            .replace(/:+.*$/gi, "") // regex 'greedy' selector anchored to string end, searching for ":" and deleting everything after
            .trim(); // END OF NORMALISATION
            // Restore escaped colons back to "\:".
            // See above comment about escaped colons.

            normalizedSelector = normalizedSelector.replace(new RegExp(unique, "g"), "\\:");
            const isMatch = await Promise.resolve(el.matches(normalizedSelector)); // Why Promise.resolve?
            //
            ...
            if ("@charset".indexOf(rule.selectorText) !== -1) {
              console.error("ERROR", rule.type, `[${trimmedSelector}]`, `[[${normalizedSelector}]]`, `(((${rule.selectorText})))`, e);
            }
          }
        }));
      }
      ...
      if (mediaIsAllowed(conditionText, options)) {
        ...
        const nestedRules = await _filterCSSRulesByElement(el, rule.rules || rule.cssRules, options, {});
...
function sheetIsAllowed(sheet, options) {
  ...
  const checkStylesheet = (sheet, sheetMatch) => {
    ...
    throw new Error(`CSS Sniff: Unknown sheet nodeName of ${sheet.ownerNode && sheet.ownerNode.nodeName} `);
  };
...
function serializeCSSRules(rules) {
  ...
  return Object.keys(rules).map(key => {
    ...
    if (!rule) {
      throw Error(`css-sniff: serializeCSSRules() key: "${key}", value: ${JSON.stringify(rule)}`);
    }
...
const unique = `css-sniff-placeholder-${Math.random().toString(16).replace(/[^0-9A-Fa-f]/gi, "")}`;